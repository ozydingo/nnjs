nnjs.GraphXY = function(canvas) {
  if (canvas.tagName !== 'CANVAS') { throw 'nnjs.GraphXY requires a canvas element'; }
...
nnjs.GraphXY.prototype = {
  ...
  draw_matrix: function(x, y, z) {
    ...
    var colormap = new nnjs.Colormap('yuletide');
    ...
    for (var ii = 0; ii < y.length; ii++) {
      ...
      for (var jj = 0; jj < x.length; jj++) {
        ...
        // debugger;
        var ww = x1 - x0;
        var color = colormap.style(z[ii][jj]);
  ...
  validate_matrix: function(x, y, z) {
    if (typeof(x) !== 'object' || typeof(x.length) !== 'number') { throw 'Invalid data for matrix: expected Array'; }
    if (typeof(y) !== 'object' || typeof(y.length) !== 'number') { throw 'Invalid data for matrix: expected Array'; }
    if (typeof(z) !== 'object' || typeof(z.length) !== 'number') { throw 'Invalid data for matrix: expected Array'; }
