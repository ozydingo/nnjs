...
nnjs.NetworkPainter.prototype = {
  ...
  neuron_y: function(layer, index) {
    if (this.network.layers[layer].length === 1) {
      ...
      return this.neuron_radius() + index * (this.canvas_height() - 2 * this.neuron_radius()) / (this.network.layers[layer].length - 1);
    }
  },

  coef_color: function(coef) {
    if (coef > 0) {
      var red = math.round(255 * (2 * nnjs.functions.sigmoid(5 * coef) - 1));
      var blue = 0;
    } else {
      var red = 0;
      var blue = math.round(255 * (2 * nnjs.functions.sigmoid(-5 * coef) - 1));
    }
